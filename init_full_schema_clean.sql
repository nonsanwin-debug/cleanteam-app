...
DROP POLICY IF EXISTS "Admins can manage submissions" ON public.checklist_submissions;
DROP POLICY IF EXISTS "Workers can manage their submissions" ON public.checklist_submissions;
DROP POLICY IF EXISTS "Workers can view submissions for their sites" ON public.checklist_submissions;
DROP POLICY IF EXISTS "Admins can manage photos" ON public.photos;
DROP POLICY IF EXISTS "Workers can manage photos" ON public.photos;
DROP POLICY IF EXISTS "Admins can manage AS requests" ON public.as_requests;
DROP POLICY IF EXISTS "Workers can view/create AS requests" ON public.as_requests;
...
DROP POLICY IF EXISTS "Admins can manage company withdrawals" ON public.withdrawal_requests;
...
CREATE POLICY "Users can view templates" ON public.checklist_templates FOR SELECT USING (
    company_id IS NULL OR company_id = (SELECT company_id FROM public.users WHERE id = auth.uid())
);
