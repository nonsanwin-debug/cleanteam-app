...
3. **[중요] 실제 사진 시각적 분석 및 삽입 규칙**:
   - 당신은 이미지 분석(Multimodal) 능력을 갖추고 있으므로, 제공된 ZIP 파일 안의 실제 이미지 파일들을 하나씩 시각적으로 직접 확인하세요.
   - **주의**: 사진 파일명에 지정된 번호(예: '01', '02', '03' 등)나 '일반'이라는 단어에 의존해 기계적으로 본문 순서에 매핑해서는 절대 안 됩니다. 작업자가 사진을 등록할 때 구역을 지정하지 않은 경우 순서대로 번호가 부여되고 '일반'이라는 명칭이 붙습니다.
   - **실제 이미지 내용 매칭**: 각 이미지의 시각적 형태를 분석하여 정확한 구역을 판별하세요:
     - 현관문, 도어락, 신발장, 보양지 등이 보인다면 **'현관'** 관련 문단에 매핑
     - 가스레인지, 싱크대 상하부장, 후드, 서랍장, 주방 수납 칸막이 등이 보인다면 **'주방/서랍장'** 관련 문단에 매핑
     - 변기, 세면대, 샤워기, 타일, 배수구 등이 보인다면 **'화장실/욕실'** 관련 문단에 매핑
     - 창문, 창틀, 바닥, 베란다 배관 등이 보인다면 **'방/거실/베란다'** 관련 문단에 매핑
   - 본문 스토리텔링에서 특정 영역(예: "주방 및 서랍장 내부 비교")을 묘사하는 곳에는 **실제로 해당 주방 가구/서랍장 또는 주방 내벽이 찍힌 사진 파일명**을 정확히 매핑하여 '[사진 삽입부: 파일명]' 형태로 표시해야 합니다. 엉뚱하게 현관문 사진을 서랍장/주방 설명 본문에 삽입하는 실수를 절대 피하세요.
   - 가급적 "작업전 사진"과 "작업후 사진"이 한 세트로 묶여서 배치될 수 있도록 묘사해 주세요.
4. **[보안 및 개인정보 보호 규칙 - 절대 준수]**:
   - **현장 팀장의 실명(이름)은 본문 글에 절대 노출하지 마세요.** 원본 데이터 상 기재된 팀장 이름은 참고용일 뿐이며, 블로그 본문에서는 오직 'NEXUS 전문 청소 팀장', '현장 베테랑 전문가', '당사 전문 홈 마스터' 등의 일반 명칭이나 업체명으로만 칭해야 합니다.
   - **아파트 및 주택의 상세 주소 중 '동, 호수'는 고객의 소중한 개인정보 보호를 위해 절대 본문 글에 기재해서는 안 됩니다.** 블로그 글에는 오직 '마포구 아파트', '송도동 신축 아파트', '서초동 OOO 아파트'와 같이 대략적인 지역명과 아파트명까지만 자연스럽게 언급해야 합니다.
5. **네이버 블로그 최적화(SEO) 팁**:
